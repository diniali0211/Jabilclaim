...
if att_file and mst_file:
    try:
        ...
        # ----- Eligibility -----
        att["CLAIM_START"] = att["JOIN_DATE"] + pd.Timedelta(days=30)
        ...
        daily = (
            eligible.groupby(
                ["__EmpID","__Name","JOIN_DATE","CLAIM_START","ELIGIBLE_END",
        ...
        for cyc in daily["CLAIM_CYCLE"].unique():
            ...
            ordered_cols = (
                ["Emp No", "Name", "JOIN_DATE", "CLAIM_START", "ELIGIBLE_END", "Recruiter"]
                + labels                              # 24-Dec → … → 26-Jan
                + ["TOTAL_WORKING", "Total_Hours"]    # totals at the END
            )

            # Safety: ensure Total_Hours exists
        ...
        with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
            for cyc in tables:
                ...
                summaries[cyc].to_excel(writer, sheet_name=sheet, startrow=len(tables[cyc]) + 2, index=False)
